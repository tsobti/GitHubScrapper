//Go to git topics page
//from there, print all 3 names and the links 

//to find the top 3 topics
...
function callback(error, response, html) {
  if (!error) {
    ...
    {
        console.log(manipulationTool(manipulationTool(allLinks[i]).find("p")[0]).text().trim()); //.find(css selector)[index]
        //the topic is in first p tag, so,  1.MT to find 1st p   2.MT to convert to text and trim 
        console.log("https://github.com"+manipulationTool(allLinks[i]).attr("href"));    //*** 
        console.log("-------------------------------------------");
    }
